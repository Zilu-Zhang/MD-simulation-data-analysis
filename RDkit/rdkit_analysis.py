    ...
    for i in range(len(value)):
        ...
        if once_more:
            ...
            for index, density in match_d.items():
                contribs[index] = density
            img = SimilarityMaps.GetSimilarityMapFromWeights(mol, contribs, colorMap='jet', contourLines=10)
            img.savefig('./%s/donor_%s.png' % (key, resi_cur), bbox_inches = 'tight')
            plt.close()
            ...
            for index, density in match_a.items():
                contribs[index] = - density
            img = SimilarityMaps.GetSimilarityMapFromWeights(mol, contribs, colorMap='jet', contourLines=10)
            img.savefig('./%s/acceptor_%s.png' % (key, resi_cur), bbox_inches = 'tight')
            plt.close()
