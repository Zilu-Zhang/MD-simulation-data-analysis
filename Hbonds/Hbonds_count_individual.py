...
for filename in os.listdir('./'):
    if filename.endswith('.pdb'):
        ...
        if not os.path.isfile('Hbonds_count_individual.xlsx'):
            df.to_excel('Hbonds_count_individual.xlsx', '%s' % excipient_name, index = True)

        else:
            excel_book = pxl.load_workbook('Hbonds_count_individual.xlsx')
            with pd.ExcelWriter('Hbonds_count_individual.xlsx', engine = 'openpyxl') as writer:
