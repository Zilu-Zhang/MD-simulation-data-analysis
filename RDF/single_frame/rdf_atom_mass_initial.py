...
for filename in os.listdir('./'):
    if filename.endswith('.pdb'):
        ...
        total = np.empty(total_length * n_frames)
        ...
        if not os.path.isfile('rdf_atom_mass_0.xlsx'):
            df.to_excel('rdf_atom_mass_0.xlsx', '%s' % excipient_name, index = True)
        
        else:
            excel_book = pxl.load_workbook('rdf_atom_mass_0.xlsx')
            with pd.ExcelWriter('rdf_atom_mass_0.xlsx', engine = 'openpyxl') as writer:
